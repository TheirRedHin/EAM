AssetQuery = {
	...
	onQueryTask: function(scan_id,bdcall){
		var where="scan_id=?";
		var sql = 'SELECT scan_state FROM assetscan WHERE ' + where + ' GROUP BY scan_state';
...
//				document.getElementById("uploadscan").setAttribute("style","display:none");
	...
	showData: function(where, pageIndex,scan_id) {
		...
		var sql = 'SELECT * FROM assetscan WHERE ' + where + ' ORDER BY device_code';
		JxDao.query(sql, [scan_id], function(json){
			json=JSON.stringify(json);
			json=JSON.parse(json);
		...
		//根据查询条件判断将HTML拼接到哪里
			...
			if(where.indexOf("isscan = '0'")==0){
				var htm ="<tr><td style='width:30%;'>设备编号</td><td style='width:52%;'>设备名称</td><td style='width:18%;'>状态</td></tr>";
...
//					if(json[i].use_state=="4")json[i].use_state="闲置";
					...
					var html="<tr><td>"+json[i].device_code+"</td><td>"+json[i].device_name+"</td><td>"+use_state+"</td></tr>"
					htm += html;
				}
				...
				mui('#item1-table tbody').on('click','tr', function (e) {
				    ...
				    if(target.tagName.toLowerCase() === "td") {
						...
						jxm.open('scan.html',{extras:{device_code:device_code,weipan:'wp'}});
				    }
				});
...
//					if(json[i].use_state=="4")json[i].use_state="闲置";
				...
				mui('#item2-table tbody').on('click','tr', function (e) {
				    ...
				    if(target.tagName.toLowerCase() === "td") {
						...
						var device_code = $("#item2-table tr:eq("+rowIdx+") td:eq(0)").text();
						console.log(device_code);
						jxm.open('scan.html',{extras:{device_code:device_code}});
				    }
				});
			}
			//盘盈
...
//					if(json[i].use_state=="4")json[i].use_state="闲置";
				...
				mui('#item3-table tbody').on('click','tr', function (e) {
				    ...
				    if(target.tagName.toLowerCase() === "td") {
						...
						var device_code = $("#item3-table tr:eq("+rowIdx+") td:eq(0)").text();
						console.log(device_code);
//						jxm.open('scan.html',{extras:{readOnly:'device_code'}});
						jxm.open('scan.html',{extras:{device_code:device_code,readOnly:'1'}});
				    }
				});
			}
			//盘亏
...
//					if(json[i].use_state=="4")json[i].use_state="闲置";
				...
				mui('#item4-table tbody').on('click','tr', function (e) {
				    ...
				    if(target.tagName.toLowerCase() === "td") {
						...
						var device_code = $("#item4-table tr:eq("+rowIdx+") td:eq(0)").text();
						console.log(device_code);
						jxm.open('scan.html',{extras:{device_code:device_code}});
				    }
				});
		...
		//修改分页信息
		var psql = 'SELECT count(*) as cnt FROM assetscan WHERE ' + where + ' ORDER BY device_code';
