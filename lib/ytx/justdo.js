...
(function() {
    window.IM = window.IM || {
        ...
        _login : function(user_account,pwd) {
            ...
            if(flag){
            	IM._privateLogin(user_account, timestamp, function(obj) {
                    ...
                    IM.jxm_EV_login(user_account,pwd, obj.sig, timestamp);
                }, function(obj) {
		...
		jxm_EV_login : function(user_account,pwd, sig, timestamp) {
            console.log("EV_login");
            ...
            RL_YTX.login(loginBuilder, function(obj) {
                ...
                alert("JXM 错误码： " + obj.code+"; 错误描述："+obj.msg);
            });
        },
        ...
        jxm_EV_sendTextMsg : function(oldMsgid, text, receiver,isresend,record) {
            console.log('send Text message: receiver:[' + receiver
                    + ']...connent[' + text + ']...');
            var obj = new RL_YTX.MsgBuilder();
            obj.setText(text);
...
*/
         ...
         * 样式，发送消息
         */
        jxm_DO_sendMsg : function(msg) {
			var im_send_content = msg.content;
            var msgid = new Date().getTime();
            var content_type = 'C';
            var content_you = msg.receiver;
            ...
            if (IM._contact_type_c == content_type) {
                IM.jxm_EV_sendTextMsg(msgid, im_send_content, content_you,false);
            };
        ...
        jxm_DO_im_image_file_up : function(receiver, img_blob) {
        	var msgid = new Date().getTime();
            var oFile = $(document).find('input[imtype="msg_attach_resend"]')[0];
