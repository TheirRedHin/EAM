(function() {
    var YTX_CONFIG = {
        _server_ip: ["MTIzLjU2LjEzNS44MTo3Njgy"],
        _file_server_url: "MTIzLjU2LjEzNS44MTo3Njgy",
        _lvs_servers: ["MTIzLjU2LjEzNS44MTo4ODg4"],
        _file_sig_url: "MTIzLjU3LjIzMC4xNTg6ODg4Ng==",
        ...
        _initScoket: function(type, callback, onError, sig, timestamp, reset) {
            //$(window).unload(function() { by tony.tan
            ...
            if (!YTX_CONFIG._isConnect) {
                if (!YTX_CONFIG._isConnecting) {
                    ...
                    YTX_CONFIG._socket = new WebSocket("ws://" + serverip);
            ...
            _parsePushMsgResp: function(obj) {
                ...
                if ( !! data["9"]) {
                    ...
                    if (!fileUrl.startWith("http")) {
                        ...
                        fileUrl = "http://" + lvsServer + fileUrl
                    }
                ...
                if ( !! data["6"]) {
                    ...
                    if ( !! msgInfos && msgInfos.length > 0) {
                        ...
                        if ( !! msgInfo["3"]) {
                            ...
                            var idx = Math.round(Rand * Range);
                            var lvsServer = Base64.decode(YTX_CONFIG._lvs_servers[idx]);
                            msg.msgFileUrl = "http://" + lvsServer + fileUrl;
                            if (msg.msgType == 3) {
                ...
                if ( !! data["6"]) {
                    ...
                    if ( !! msgInfos && msgInfos.length > 0) {
                        ...
                        if ( !! msgInfo["3"]) {
                            ...
                            var idx = Math.round(Rand * Range);
                            var lvsServer = Base64.decode(YTX_CONFIG._lvs_servers[idx]);
                            m.msgFileUrl = "http://" + lvsServer + fileUrl;
                            if (m.msgType == 3) {
                ...
                for (var i in data) {
                    ...
                    if (orign.msgType == YTX_CONFIG._prototype._msg_CMD) {
                        ...
                        if ( !! orign.msgFileUrl) {
                            ...
                            var idx = Math.round(Rand * Range);
                            var lvsServer = Base64.decode(YTX_CONFIG._lvs_servers[idx]);
                            result.msgFileUrl = "http://" + lvsServer + fileUrl;
                            if (result.msgType == 3) {
        ...
        _sendFile: function(fileInfo, content, receiver, msgType, msgId, msgDomain, type, callback, onError, progress, fileName, orignMsgId) {
            ...
            var ws = new WebSocket("ws://" + fileurl);
                        ...
                        if (bodyArr[j].indexOf("fmtp") > 0) {
                            ...
                            if (bodyArr[j].indexOf("candidate") > 0) {
                                ...
                                if (bodyArr[j].indexOf("c=") == 0) {
                                    ...
                                    if (bodyArr[j].indexOf("rtcp-fb") > 0) {
                                        if (bodyArr[j].indexOf("nack") > 0 || bodyArr[j].indexOf("goog-remb") > 0) {
                                            continue
                                        }
                                    } else {
                                        newBody += tabStr + bodyArr[j]
    ...
    window.RL_YTX = window.RL_YTX || {
        init: function(appid, serverIp, lvsServer, fileSig) {
            ...
            if ( !! serverIp) {
                ...
                YTX_CONFIG._file_server_url = Base64.encode("ws://" + serverIp)
            }
            if ( !! lvsServer) {
